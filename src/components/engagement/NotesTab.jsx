...
/**
 * Format date for note display: "Jan 8" or "Jan 8, 2024" if not current year
 */
...
const NoteCard = ({ 
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-3 group hover:border-gray-300 transition-colors">
      {/* Note header: date, author initials, edited indicator */}
...
const AddNoteInput = ({ onAdd, phaseType }) => {
  ...
  return (
    <div ref={containerRef} className="bg-gray-50 rounded-lg p-3">
      <textarea
        ref={textareaRef}
        value={text}
        onChange={handleTextChange}
...
/**
 * Phase notes section (collapsible)
 */
const PhaseNotesSection = ({ 
  ...
  return (
    <div 
      ref={scrollRef}
      className={`border rounded-lg overflow-hidden ${isPending && !hasNotes ? 'opacity-60' : ''}`}
    >
      {/* Header */}
      <button
        onClick={onToggle}
...
/**
 * Notes tab showing all phase notes
 * 
 * @param {Object} props
 ...
 * @param {string} props.scrollToPhase - Phase type to scroll to
 */
const NotesTab = ({ 
  engagement, 
  getOwnerInfo, 
  ...
  // Track expanded state per phase
  const [expandedPhases, setExpandedPhases] = useState(() => {
    // Default: expand phases with notes
    const expanded = {};
    phaseConfig.forEach(p => {
      const hasNotes = engagement?.notesByPhase?.[p.id]?.length > 0;
      expanded[p.id] = hasNotes;
    });
    return expanded;
  });
  ...
  useEffect(() => {
    if (scrollToPhase) {
      ...
      setTimeout(() => {
        scrollRefs.current[scrollToPhase]?.scrollIntoView({ 
          behavior: 'smooth', 
          block: 'start' 
        });
      }, 100);
  ...
  // Re-calculate expanded state when engagement changes
  useEffect(() => {
    setExpandedPhases(prev => {
      const next = { ...prev };
      phaseConfig.forEach(p => {
        const hasNotes = engagement?.notesByPhase?.[p.id]?.length > 0;
        // Only auto-expand if not manually collapsed and has notes
        if (hasNotes && prev[p.id] === undefined) {
          next[p.id] = true;
        }
      });
      return next;
    });
  }, [engagement?.notesByPhase]);

  const togglePhase = (phaseType) => {
    setExpandedPhases(prev => ({ ...prev, [phaseType]: !prev[phaseType] }));
  };
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      {phaseConfig.map((phaseInfo, idx) => {
        const notes = engagement?.notesByPhase?.[phaseInfo.id] || [];
