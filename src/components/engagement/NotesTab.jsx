...
import { PencilIcon, TrashIcon, PlusIcon, ChevronRightIcon } from '../ui';
...
const NoteCard = ({ 
  ...
  if (isEditing) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-blue-300 dark:border-blue-600 p-3">
        <textarea
          ref={textareaRef}
          value={editText}
          onChange={(e) => {
          ...
          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500"
          rows={3}
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 p-3 group hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span
            className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-medium ${
              author.isSystemUser ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
            }`}
            title={author.name}
          >
            {author.initials}
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="w-full py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg flex items-center justify-center gap-1 transition-colors"
      >
    ...
    <div ref={scrollRef} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <ChevronRightIcon className={`w-4 h-4 text-gray-500 dark:text-gray-400 transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
          <span className={`font-medium ${isLabelMuted ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-gray-100'}`}>
            {phaseInfo.label}
          </span>
          {hasNotes && (
      ...
      {isExpanded && (
        <div className="p-4 space-y-3 bg-white dark:bg-gray-900">
          {hasNotes ? (
            notes.map((note) => (
              <React.Fragment key={note.id}>
                ...
                {showDeleteConfirm?.id === note.id && (
                  <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg p-3 flex items-center justify-between">
                    <span className="text-sm text-red-700 dark:text-red-300">Delete this note?</span>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-2">No notes yet</p>
          )}
